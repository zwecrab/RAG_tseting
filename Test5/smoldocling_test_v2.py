...
def main():
    ...
    # Use your specific PNG file from Downloads folder
    import os
    downloads_folder = os.path.join(os.path.expanduser("~"), "Downloads")
    image_path = os.path.join(downloads_folder, "Format_FinancialFormat 24.2.png")
    ...
    if not os.path.exists(image_path):
        ...
        print("\nPlease check:")
        print("1. File name is exactly: 'Format_FinancialFormat 24.2.png'")
        print("2. File is in your Downloads folder")
        print("3. File extension is .png")
        print("\nAlternatively, you can:")
        print("- Move the file to the same directory as this script")
        print("- Update the image_path variable with the correct path")
        return
    
    print(f"✓ File found: {image_path}")
    print(f"File size: {os.path.getsize(image_path) / 1024:.2f} KB")
    
    # Extract tables using different prompts specifically for financial documents
    prompts = [
        "Extract the complete financial forecasting table with all headers and data",
        "Convert this financial planning spreadsheet to structured format",
        "Extract table preserving row and column labels",
        "Convert this financial table to OTSL format with all data"
    ]
    ...
    for prompt_idx, prompt in enumerate(prompts):
        ...
        tables = extractor.extract_tables_from_image(image_path, prompt)
        ...
        if tables:
            ...
            # Save tables to files with descriptive name
            output_dir = f"output_financial_format_{prompt_idx + 1}"
            ...
            doc = extractor.convert_to_docling_document(image_path, tables)
            if doc:
                ...
                markdown_file = f"{output_dir}/financial_format_document.md"
    ...
    print("='*80}")
    print("Check the output_financial_format_* directories for extracted table files!")
    print("Files generated:")
    print("  - table_X.otsl (OTSL format)")
    print("  - table_X_metadata.json (location and metadata)")
    print("  - table_X_raw_doctags.txt (raw model output)")
    print("  - financial_format_document.md (Markdown export)")
